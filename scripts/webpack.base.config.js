...
const webpackConfigBase = {
  ...
  resolve: {
    ...
    alias: {
      ...
      '@store': path.join(__dirname, '../app/stores/store.js'),
      '@actions': path.join(__dirname, '../app/stores/actions'),
      '@reducers': path.join(__dirname, '../app/stores/reducers'),
      ...
      '@util': path.join(__dirname, '../app/utils/index.js'),
  ...
  module: {
    rules: [
      ...
      },
      {
        ...
        exclude: /node_modules/,
        include: [resolve('../app/assets/images')],
        loader: 'url',
        options: {
          limit: 8192,
  ...
  plugins: [
    ...
    new HappyPack({
      ...
      id: 'happyStyle',
      //如何处理  用法和loader 的配置一样
      loaders: [ 'css-loader?sourceMap=true&minimize=true', 'less-loader?sourceMap=true' ], 
      //代表共享进程池，即多个 HappyPack 实例都使用同一个共享进程池中的子进程去处理任务，以防止资源占用过多。
      threadPool: happyThreadPool,
      //允许 HappyPack 输出日志
      verbose: true,
